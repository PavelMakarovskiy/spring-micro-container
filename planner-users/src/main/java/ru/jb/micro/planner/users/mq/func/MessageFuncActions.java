...
public class MessageFuncActions {
    ...
    public void sendNewUserMessage(Long id) {
        ...
        messageFuncProduce.getInnerBusUserSide().emitNext(MessageBuilder.withPayload(id).build(), Sinks.EmitFailureHandler.FAIL_FAST);
        System.out.println("Message sent: " + id);
    }

    public void sendNewOrderUserSide(Order order) {
        messageFuncProduce.getInnerOrderBusUserSide().emitNext(MessageBuilder.withPayload(order).build(), Sinks.EmitFailureHandler.FAIL_FAST);
    }

    public void sendNewWsOrderUserSide(Order order) {
        messageFuncProduce.getWsOrderBusUserSide().emitNext(MessageBuilder.withPayload(order).build(), Sinks.EmitFailureHandler.FAIL_FAST);
    }
}
